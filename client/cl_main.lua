...
local faceProps = {
    ...
    [4] = {
        ...
        ["Palette"] = -1,
        ["Texture"] = -1
    }, -- En realidad, esto es una variación de pedtexture, no un accesorio.
    [5] = {
        ["Prop"] = -1,
        ["Palette"] = -1,
        ["Texture"] = -1
...
local function resetClothing(data)
    ...
    -- Rostro
    SetPedHeadBlendData(ped, data["face"].item, data["face2"].item, nil, data["face"].texture, data["face2"].texture,
        nil, data["facemix"].shapeMix, data["facemix"].skinMix, nil, true)
...
local function openMenu(allowedMenus)
    ...
    SendNUIMessage({
        ...
        allowPlayerModels = Config.AllowPlayerModels
    })
...
local function ChangeVariation(data)
    ...
    elseif clothingCategory == "face" then
        ...
        elseif type == "texture" then
            SetPedHeadBlendData(ped, skinData["face"].item, skinData["face2"].item, nil, item,
                skinData["face2"].texture, nil, skinData["facemix"].shapeMix, skinData["facemix"].skinMix, nil, true)
...
local function reloadSkin(health)
    ...
    if gender == 1 then -- El género es UNO para MUJER
        model = GetHashKey("mp_f_freemode_01") -- Modelo femenino
    else
        model = GetHashKey("mp_m_freemode_01") -- Modelo masculino
    end
    ...
    Citizen.Wait(1000) -- Retraso de seguridad

    TriggerServerEvent("qb-clothes:loadPlayerSkin") -- CARGANDO LA ROPA DEL JUGADOR
    TriggerServerEvent("qb-clothing:loadPlayerSkin") -- CARGANDO ROPA DE JUGADOR - Evento 2
...
local function getOutfits(gradeLevel, data)
    ...
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({{
            ...
            menu = "accessoires",
            label = Lang:t("menu.accessoires"),
            selected = false
        }, {
            menu = "myOutfits",
            label = Lang:t("outfits.myOutfits"),
            selected = false,
            outfits = result
        }, {
            menu = "roomOutfits",
            label = Lang:t("outfits.roomOutfits"),
            selected = false,
            outfits = data[gender][gradeLevel]
        }})
    end)
end
...
RegisterNetEvent('onResourceStart', function(resourceName)
    ...
    PlayerData = QBCore.Functions.GetPlayerData()
    -- Actualiza los puntos/zones aquí si es necesario
    loadStores()
end)
...
RegisterNetEvent('qb-clothing:client:openMenu', function()
    ...
    openMenu({{
        menu = "character",
        label = Lang:t("menu.features"),
        selected = true
    }, {
        menu = "hair",
        label = Lang:t("menu.hair"),
        selected = false
    }, {
        menu = "clothing",
        label = Lang:t("menu.character"),
...
RegisterNetEvent('qb-clothing:client:loadOutfit', function(oData)
    ...
    for k in pairs(data) do
        ...
        -- Para garantizar la compatibilidad con versiones anteriores de facemixing
    ...
    if oData.outfitName ~= nil then
        QBCore.Functions.Notify("Has seleccionado el outfit " .. oData.outfitName .. ".")
    end
end)
RegisterNetEvent("qb-clothing:client:adjustfacewear", function(type)
    ...
    if removeWear then
        ...
        if type ~= 5 and type ~= 2 then
            if type == 4 then
                SetPedComponentVariation(PlayerPedId(), PropIndex, faceProps[type]["Prop"], faceProps[type]["Texture"],
                    faceProps[type]["Palette"])
            else
                SetPedPropIndex(PlayerPedId(), tonumber(PropIndex), tonumber(faceProps[PropIndex + 1]["Prop"]),
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    QBCore.Functions.Notify(Lang:t('notify.info_deleteOutfit', {
        outfit = data.outfitName
    }))
    cb('ok')
end)
...
-- Lo definimos como función para que no se obtenga un valor nulo para el trabajo. La función se activa al cargar el reproductor. :)
function loadStores()
        ...
        CreateThread(function()
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not QBCore.Shared.QBJobsStatus then
                    ...
                    break
                end -- esta interrupción es importante si QBJobsStatus es verdadero, entonces no es necesario ejecutar el resto del código en este bucle.
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'clothing' then
                        exports['DP-TextUI']:MostrarUI('clothing_textui', Lang:t("store.clothing"), 'E', false)
                    elseif zoneName == 'barber' then
                        exports['DP-TextUI']:MostrarUI('barber_textui', Lang:t("store.barber"), 'E', false)
                    elseif zoneName == 'outfit' then
                        exports['DP-TextUI']:MostrarUI('outfit_textui', Lang:t("store.outfitchanger"), 'E', false)
                    end
                else
                    inZone = false
            ...
            if PlayerData.gang and PlayerData.gang.name or (not QBCore.Shared.QBJobsStatus and PlayerData.job.name) then
                ...
                clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                    if isPointInside then
                        ...
                        local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or
                                        (not QBCore.Shared.QBJobsStatus and PlayerData.job.name)
                        if (job == Config.ClothingRooms[zoneID].requiredJob) then
                            zoneName = zoneID
                            inZone = true
                            exports['DP-TextUI']:MostrarUI('room_clothing_textui', Lang:t("store.clothing"), 'E', false)
                        end
                    else
                        inZone = false
                        exports['DP-TextUI']:OcultarUI(id)
                ...
                if inZone then
                    ...
                    if zoneName == 'clothing' then
                        if IsControlJustReleased(0, 38) then
                            customCamLocation = nil
                            QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(myOutfits)
                                openMenu({{
                                    menu = "clothing",
                                    label = Lang:t("menu.character"),
                                    selected = true
                    ...
                    elseif not QBCore.Shared.QBJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or
                                                   (not QBCore.Shared.QBJobsStatus and PlayerData.job.grade.level)
...
---¿El modelo es alguno de los modelos de freemode?
---@param modelHash integer
local function isFreemodeModel(modelHash)
    return freemodeModels[modelHash] ~= nil
end

---Obtiene datos de mezcla de cabezas
---@param ped integer
---@return HeadBlendData
local function getHeadBlendData(ped)
    -- GTA devuelve una estructura tonta con punteros.
    -- Esta es una forma un tanto peculiar de obtener los datos en Lua.
    -- Como alternativa, necesitarías un script en C# o JS para obtener los datos. Sin embargo, esto es mucho menos trabajo.
    -- Se ha comentado esto aquí:
    -- https://forum.cfx.re/t/head-blend-data/212575/24
    local tbl<const> = {Citizen.InvokeNative(0x2746BD9D88C5C5D0, ped, Citizen.PointerValueIntInitialized(0),
...
---Restaura los datos de mezcla guardados
---@param ped integer
local function restoreSavedBlendData(ped)
    SetPedHeadBlendData(ped, savedBlendData.shapeFirst, savedBlendData.shapeSecond, savedBlendData.shapeThird,
        savedBlendData.skinFirst, savedBlendData.skinSecond, savedBlendData.skinThird, savedBlendData.shapeMix,
        savedBlendData.skinMix, savedBlendData.thirdMix, false)
end

---Restaura las características faciales guardadas
---@param ped integer
...
---Reducir los rasgos faciales
---@param ped integer
local function shrinkFaceFeatures(ped)
    repeat
        Wait(0)
    until HasPedHeadBlendFinished(ped)
    for i = 0, 19 do
...
---Cabeza de peatón encogida
---@param ped integer
---@param pedModelHash integer
local function shrinkHead(ped, pedModelHash)
    SetPedHeadBlendData(ped, freemodeModels[pedModelHash] == 'mp_m_freemode_01' and 0 or 21, 0, 0, -- Restablecer forma
    savedBlendData.skinFirst, savedBlendData.skinSecond, savedBlendData.skinThird, -- Mantener la piel
    0.0, savedBlendData.skinMix, 0.0, -- Restablecer todo excepto la mezcla de pieles
...
---Maneja la corrección de la máscara
---@param ped integer
---@param pedModelHash integer
local function fixMask(ped, pedModelHash)
    ...
    if currentMaskDrawable > 0 then
        ...
        if DoesShopPedApparelHaveRestrictionTag(maskHash, 'SHRINK_HEAD', 0) or -- MÁSCARAS CODIFICADAS
        currentMaskDrawable == 108 -- Esta maldita máscara de calavera no tiene etiqueta de restricción, pero necesita encoger la cabeza.
        or currentMaskDrawable == 30 -- Lo mismo ocurre con esta máscara de hockey. No tiene etiquetas, pero se engancha en la nariz.
        then
        ...
        if not DoesShopPedApparelHaveRestrictionTag(maskHash, 'HAT', 0) and
            not DoesShopPedApparelHaveRestrictionTag(maskHash, 'EAR_PIECE', 0) -- MÁSCARAS CODIFICADAS
        and not (currentMaskDrawable == 11 -- Gay super hero
        -- o currentMaskDrawable == 73 -- Sin máscara (esto permanecerá como está - podría ser útil colocarlo debajo de cascos específicos)
        or currentMaskDrawable == 114 -- Open face headscarf
        -- o currentMaskDrawable == 120 -- Sin máscara calva (esto permanecerá como está - podría ser útil colocarlo debajo de cascos específicos)
...
-- Generalmente con FiveM, los jugadores pueden cambiar su ropa libremente y sobre la marcha.
-- Idealmente, se debería arreglar la ropa justo después de que el jugador se cambie.
-- Sin embargo, como hay mil maneras diferentes de cambiarse de ropa, he decidido arreglar la ropa en un bucle.
-- Sugiero reemplazar esto con un controlador de eventos para el sistema de ropa.
CreateThread(function()
    while true do
        fixClothing()
        Wait(10)
