...
$(document).on('keydown', function (event) {
    // Usamos 'event.which' para mayor compatibilidad de navegadores
    switch (event.which) {
        ...
        case 37: // Flecha izquierda
            ...
            break;
        case 27: // Escape
            // Esto simula un clic en el botón de cancelar para cerrar el menú.
            $.post('https://qb-clothing/closeMenu');
            break;
    }
});
...
// Función para recargar los atuendos
QBClothing.ReloadOutfits = function (outfits) {
    $(".clothing-menu-myOutfits-container").html("");
    $.each(outfits, function (index, outfit) {
        // Se añade la clase "my-outfits-option" al contenedor principal para aplicar los estilos de Flexbox
        var elem = '<div class="clothing-menu-option my-outfits-option" data-myOutfit="' + (index + 1) + '"> <div class="clothing-menu-myOutfit-option-button-remove"><p data-tkey="delete"><i class="fa-solid fa-trash-can"></i></p></div><div class="clothing-menu-option-header"><p>' + outfit.outfitname + '</p></div><div class="clothing-menu-myOutfit-option-button"><p data-tkey="select"><i class="fa-solid fa-check"></i></p></div></div>'
...
QBClothing.Open = function (data) {
    ...
    $("#action-indicators").css("display", "flex");
    ...
    $.each(data.menus, function (i, menu) {
        ...
        if (menu.menu == "roomOutfits") {
            $.each(menu.outfits, function (index, outfit) {
                var elem = '<div class="clothing-menu-option" data-outfit="' + (index + 1) + '"> <div class="clothing-menu-option-header"><p>' + outfit.outfitLabel + '</p></div> <div class="clothing-menu-outfit-option-button"><p data-tkey="select">Seleccionar outfit</p></div> </div>'
        ...
        // Bloque para el menú "myOutfits"
        if (menu.menu == "myOutfits") {
            $.each(menu.outfits, function (index, outfit) {
                // Se añade la clase "my-outfits-option" aquí también
                var elem = '<div class="clothing-menu-option my-outfits-option" data-myOutfit="' + (index + 1) + '"> <div class="clothing-menu-myOutfit-option-button-remove"><p data-tkey="delete"><i class="fa-solid fa-trash-can"></i></p></div><div class="clothing-menu-option-header"><p>' + outfit.outfitname + '</p></div><div class="clothing-menu-myOutfit-option-button"><p data-tkey="select"><i class="fa-solid fa-check"></i></p></div></div>'
                $(".clothing-menu-myOutfits-container").append(elem)
...
QBClothing.Close = function () {
    ...
    $(".clothing-menu-container").css({ "display": "block" }).animate({ right: "-25vw", }, 200, function () {
        ...
        $("#action-indicators").css("display", "none");
...
QBClothing.SetMaxValues = function (maxValues) {
    $.each(maxValues, function (i, cat) {
        if (cat.type == "character") {
            ...
            $(itemMax).html("<p>" + maxValues[containers.data('type')].item + " <span data-tkey='item'>Elementos</span></p>");
            ...
            $(headerMax).html("<p>" + maxValues[containers.data('type')].texture + " <span data-tkey='texture'>Texturas</span></p>");
